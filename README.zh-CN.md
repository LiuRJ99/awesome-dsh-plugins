...
| [`dsh-decision-engine`](https://github.com/LiuRJ99/dsh-decision-engine) | 面向 DSH 的模型中立低延迟决策层：提供可插拔决策引擎与 Provider、有限候选集决策协议，以及 Browser / Computer / Custom 环境适配器 | Git tag `v0.4.9`（无 GitHub Release） | DSH peer 服务；可选 `@receptron/laya` 提供本地推理；若启用 browser/computer 适配器需相应宿主工具/插件支持 |
...
dsh plugin --profile <candidate-profile> add \
  "github:LiuRJ99/dsh-decision-engine#4bf53f40689e411ddac2886e6ecb5f2513f0c471"
dsh plugin --profile <candidate-profile> add \
