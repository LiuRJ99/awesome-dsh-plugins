...
在本地开发环境中，亦可直接使用工作区软链接挂载至 profile（`dsh plugin --profile web link <path>`）。
...
本清单中需要保留自主开发内容的 LiuRJ99 fork 包括：CPA（`dsh-cpa-plugin`）、
WorkBuddy（`dsh-workbuddy-provider`）、浏览器桥接（`dsh-browser`）、
Computer Use（`dsh-computer-use`）、图片生成（`dsh-image-gen`）、
用量统计（`dsh-spend`）、taskboard（`dsh-taskboard-cloader`）和 lazy-gate（`dsh-tool-lazy-gate`）。
`dsh-better-sidebar`、`dsh-mobile` 与 `dsh-sandbox-schema-shim` 不在此列：
只使用上游或作者发布包，不代表本地维护或修改上游项目。
